...
- **Phase 9 — macOS 네이티브 앱** ✅ **구현 완료(2026-07-30), 실기기(실제 Spotify 재생 연동) 마일스톤 검증은 대기**: 웹·TUI와 동일하게 `api/`만 호출하는 독립 클라이언트, Phase 8.5의 상시 중앙 api에 접속. 메뉴바(재생 제어·세션 상태·실시간 코드) + 별도 라이브러리 창(트랙 목록) + 설정(api 주소). 네트워킹·모델·상태 로직은 순수 Swift 패키지 `MusicnaKit`(SwiftUI/AppKit 무의존, Phase 10과 공유 가능)로 분리 — 27 tests 통과, Xcode 앱 타겟(XcodeGen `project.yml`)도 빌드 성공 확인. 설계: [2026-07-29-phase-9-macos-native-app-design.md](superpowers/specs/2026-07-29-phase-9-macos-native-app-design.md), 구현 계획: [2026-07-29-phase-9-macos-native-app.md](superpowers/plans/2026-07-29-phase-9-macos-native-app.md). 남은 건 실제 메뉴바가 실제 Spotify 재생·캡처·라이브러리에 반응하는지의 실기기 마일스톤 검증뿐(같은 패턴: 기능 동등화, 단계별 실기기 검증)
- **Phase 10 — iOS 앱** (범위 확정, 2026-07-26): `api/`만 호출하는 라이브러리 뷰어·원격 제어(SwiftUI) 겸, **자체 Spotify Connect 재생·캡처 기기**(신규) — `librespot-golang`+gomobile 임베딩으로 앱이 포그라운드로 열려 있는 동안 직접 재생하며 디코딩 PCM을 원격 인제스트로 스트리밍(iOS는 시스템 오디오 캡처 API 자체가 없어 "자체 재생 중 버퍼 분기" 방식만 가능, 백그라운드 상주는 iOS 정책상 미지원). 재생 중엔 `/ws/live`를 네이티브 SwiftUI로 구독해 실시간 코드·피아노 롤 표시(웹 `live.html`과 동일 이벤트 계약 재사용). **착수 전 선행 조건**: `remote_capture.py`에 `session_id`별 동시성 lock 추가(위 리스크 절 참조). 설계·타당성 조사: [docs/superpowers/specs/2026-07-26-central-deployment-ios-player-design.md](superpowers/specs/2026-07-26-central-deployment-ios-player-design.md)
