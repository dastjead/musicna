...
- **현재 Phase**: Phase 0~8.5·"코드 진행 추상화 구조"·"트랙 저장·보존 정책"·Phase 8(TUI 기능 동등화)·Phase 10 선행조건(remote_capture 동시성 lock)·**Phase 9(macOS 네이티브 앱) 구현**·**Alembic 마이그레이션 도입**까지 전부 `main`에 병합 완료(각각 2026-07-30). Phase 9는 `MusicnaKit` 순수 Swift 패키지(Models·APIClient·LiveEventClient·3개 스토어, 28 tests 통과) + XcodeGen 기반 App 타겟(MenuBarExtra·라이브러리 창·설정, `xcodebuild` 빌드 성공 확인)으로 구성 — 최종 전체 브랜치 리뷰(opus) → fix wave(커밋 `05e4cb5`, 스토어 미시작 Critical 1건·볼륨/이전곡 컨트롤 부재·설정 셀렉터 미동작 Important 2건 수정) → 재리뷰 통과 후 병합, 브랜치 `phase-9-macos-native-app` 로컬·원격 삭제. **남은 건 실기기(macOS) 마일스톤 검증뿐**(실제 Spotify 재생에 메뉴바가 반응하는지 등, fix wave가 고친 3건 포함 — 이 세션엔 GUI 자동화 도구가 없어 미실시, 아래 Phase 9 체크리스트 참조). **Phase 10(iOS)은 스파이크 진행 중, 세션 중단 상태** — 아래 "Phase 10 — iOS 앱" 절의 "스파이크 재개 체크리스트" 참조(사용자 응답 대기 지점에서 중단, 워크스페이스 변경 없음·리포지토리에 커밋된 코드 없음)
...
- **작업 브랜치**: `main`은 위 Phase 9 병합까지 전부 최신. 현재 진행 중인 미병합 Phase 브랜치 없음
...
- **다음 할 일 (원격 전용 작업이 필요하다면)**: 없음 — Alembic 마이그레이션 도입 완료(`main` 병합, 기존 `data/musicna.db`도 `alembic stamp head` 적용 완료. 아래 작업 로그·Phase 4 체크리스트 참조)
...
3. `uv run pytest core/tests api/tests tui/tests` → 237 passed, 0 skipped 기대(2026-07-30 Alembic 마이그레이션 도입 완료 기준 최신 수치, `main` 기준 — extras 전부 설치된 이 머신에서는 스킵 없음)
...
5. Phase 8·8.5·9는 구현·리뷰·`main` 병합까지 완료 — 남은 건 "다음 할 일 (macOS)"의 실기기 마일스톤 검증뿐. macOS 앱을 다시 빌드하려면 `xcode-select -p`로 활성 Xcode가 여전히 잡혀 있는지 먼저 확인할 것(외장 볼륨 마운트 필요)
...
- [x] Alembic 마이그레이션 도입 — `core/src/musicna_core/store/migrations/`, `create_session_factory()`가 `create_all()` 대신 `alembic upgrade head`를 적용(`create_all` 호출은 완전히 제거, 공개 시그니처는 그대로). 초기 리비전(`3f5636cb793d`)이 기존 7개 테이블을 autogenerate로 그대로 재현(리뷰에서 컬럼 단위 대조, 손보정 불필요). 회귀 테스트: `core/tests/test_migrations.py`(2건). `main` 병합 완료(2026-07-30). 기존 `data/musicna.db`(실캡처 데이터 보관)도 백업 후 `alembic stamp head` 적용 완료 — `alembic_version` 테이블에 `3f5636cb793d` 1행 기록, 실제 ALTER 없음, 기존 7개 테이블·데이터(트랙 2·분석 8) 그대로 확인, `create_session_factory("data/musicna.db")` 직접 호출로 에러 없이 통과(멱등적 upgrade head) 검증
...
> 설계: [2026-07-29-phase-9-macos-native-app-design.md](superpowers/specs/2026-07-29-phase-9-macos-native-app-design.md). 구현 계획: [2026-07-29-phase-9-macos-native-app.md](superpowers/plans/2026-07-29-phase-9-macos-native-app.md)(Task 1~9, Task 9는 이 문서 갱신). Task 1~8 구현 + 최종 전체 브랜치 리뷰(opus) → fix wave(2026-07-30, 커밋 `05e4cb5`: Critical 1건 — 두 스토어가 launch 시 한 번도 start() 안 됨, Important 2건 — 볼륨/이전곡 컨트롤 부재·설정 셀렉터가 이 머신 OS에서 미동작 — 전부 수정) → 스코프된 재리뷰 통과 → `main`으로 fast-forward 병합, 브랜치 `phase-9-macos-native-app` 로컬·원격 삭제. 커밋(오래된 순): `da47ba9`(Task1) `63a9cc5`(Task2) `1e22b6a`(Task3) `f9cf7d8`+`c8fc508`(Task4) `e23227f`(Task5) `9289cbb`(Task6) `7aec127`(Task7) `ad981e4`(Task8) `05e4cb5`(fix wave). fix wave 상세 리뷰 기록은 SDD 워크스페이스(`.superpowers/sdd/`, git-ignored)에 있었으나 완료 후 관례대로 정리됨 — 상세는 아래 작업 로그 참조.
- [x] `MusicnaKit` 패키지(순수 Swift, SwiftUI/AppKit 무의존) — `Models.swift`(PlayerStatus/SystemStatus/TrackMeta/MoodTag/AnalysisResult/LiveEvent)·`APIClient.swift`(REST)·`LiveEventClient.swift`(WebSocket, 재연결 busy loop 방지)·`PlayerStatusStore`/`LiveAnalysisStore`/`LibraryStore`(ObservableObject 스토어) — `cd macos-app/MusicnaKit && swift test` 28 tests 통과(fix wave에서 `isConnected` 회귀 테스트 1건 추가)
...
- [x] `MenuBarView`(재생/일시정지·이전곡·다음곡·볼륨 슬라이더·세션 상태·실시간 코드) + `LibraryWindowView`(트랙 목록 Table) + `PreferencesView`(api 주소 설정, UserDefaults)
- [x] `MusicnaApp.init()`에서 `PlayerStatusStore.start()`/`LiveAnalysisStore.start()`를 즉시 호출(launch 시 폴링·WS 구독 시작 보장)
- [ ] **(macOS)** 마일스톤: 메뉴바에서 재생/일시정지·이전곡·다음곡·볼륨 조작 시 실제 Spotify 재생 반응, 캡처 상태·실시간 코드가 실제 재생과 함께 갱신, 라이브러리 창에 실제 캡처 트랙 표시 — 이 세션엔 네이티브 macOS 앱을 조작할 GUI 자동화 도구가 없어 미실시, 다음 세션에서 사용자 또는 GUI 도구로 확인 필요(fix wave가 수정한 3건 포함해 전부 미검증)

### Phase 10 — iOS 앱
> 설계·타당성 조사(2026-07-26, 원안): [2026-07-26-central-deployment-ios-player-design.md](superpowers/specs/2026-07-26-central-deployment-ios-player-design.md) — cpal iOS 지원 확인, `librespot-golang`/gomobile 레퍼런스 발견, iOS 백그라운드 정책 리스크 조사 포함.
> **⚠️ 범위 정정(2026-07-31, 스파이크 도중 확정)**: 원안은 "iOS 앱이 Spotify Connect 수신 기기가 되어 공식 Spotify 앱에서 캐스팅"하는 구조였으나, 스파이크로 `librespot-golang`의 실제 코드를 추적한 결과 **이 구조는 이 라이브러리로 불가능하다고 확인됨** — gomobile 바인딩 대상인 `librespotmobile` 패키지는 로그인·메타데이터 조회·트랙 다운로드만 노출하고, 진짜 Connect 프로토콜(SPIRC) 수신·재생 명령 처리 로직 자체가 라이브러리 어디에도 구현돼 있지 않음(`spirc.Controller`는 반대로 "남의 Connect 기기를 조종하는 리모컨"). 사용자와 논의 후 **범위를 "iOS 앱 자체가 미니 Spotify 플레이어가 되어 직접 곡을 선택·재생하고, 그 디코딩 PCM을 원격 인제스트로 스트리밍"으로 변경 확정** — Connect 캐스팅(공식 앱에서 소스 전환)은 포기하되 "아이폰이 재생하는 음악을 분석"이라는 핵심 목표는 동일하게 달성. 상세 비교(2번 vs 3번 대안)는 이 작업 로그의 2026-07-31 항목 참조. **설계 스펙 문서 자체는 아직 갱신 전** — 다음 세션에서 이 정정을 반영해 재작성 필요.

#### 스파이크 재개 체크리스트 (2026-08-12 시점, 세션 중단 지점)

**진행 상황**: `librespot-golang`(원본, gomobile 바인딩 아님) 순수 Go 레벨에서 실제 Spotify 서버와의 로그인 프로토콜 핸드셰이크까지는 검증 완료. 남은 건 사용자의 올바른 자격 증명으로 로그인 성공 확인 → 트랙 다운로드·Vorbis 디코딩 검증 → gomobile/iOS 단계로 넘어가는 것.

1. 스파이크 코드는 레포 밖 스크래치 디렉터리에 있음(세션마다 경로가 바뀌므로 이 문서에 고정 경로를 적지 않음) — **다음 세션에서는 새로 clone해서 아래 패치부터 다시 적용**:
   - `go install golang.org/x/mobile/cmd/gomobile@latest` + `gomobile@latest`(gobind) — 이미 설치돼 있었다면 `gomobile version` 확인 후 생략 가능
   - `git clone --depth 1 https://github.com/librespot-org/librespot-golang.git`
   - **필수 패치 1**: `librespot/core/session.go`의 `makeHelloMessage()`에서 `Product: Spotify.Product_PRODUCT_PARTNER.Enum()` → `Spotify.Product_PRODUCT_CLIENT.Enum()`로 변경 — 원본 그대로면 서버가 핸드셰이크 도중 `unexpected EOF`로 연결을 끊음(내부/파트너 전용 클라이언트 타입이라 거부당하는 것으로 추정). 이 패치 후 서버가 정상적인 `APLoginFailed`/`APWelcome` 응답을 주기 시작함
   - **필수 패치 2**: `librespot/core/login.go`의 `handleLogin()`이 `PacketAuthFailure`를 받았을 때 그냥 `"authentication failed"`로 뭉개고 실제 이유(`Spotify.APLoginFailed`의 `ErrorCode`/`ErrorDescription`/`RetryDelay`)를 버림 — `proto.Unmarshal(data, &Spotify.APLoginFailed{})` 후 그 필드들을 에러 메시지에 포함하도록 패치해야 실패 원인이 보임(BadCredentials인지 2FA 요구인지 등 구분 안 되면 디버깅 불가)
   - `brew install portaudio libogg libvorbis` (main.go의 예제 CLI가 CGO로 이것들을 링크함)
   - 빌드: `PKG_CONFIG_PATH="$(brew --prefix libogg)/lib/pkgconfig:$(brew --prefix libvorbis)/lib/pkgconfig" CGO_CFLAGS="-I$(brew --prefix portaudio)/include" CGO_LDFLAGS="-L$(brew --prefix portaudio)/lib" go build -o /tmp/microclient .`
2. **검증된 사실**: `apresolve.spotify.com`이 정상 응답하고 반환하는 AP 후보 전부 이 머신에서 TCP 접속 가능(연결 실패 시 단순 재시도로 해결 — 라이브러리가 실패 시 재시도를 안 함, 후속 스파이크에서 이것도 감안). 패치 2건 적용 후 실제 계정으로 로그인 시도 시 서버가 `authentication failed: code=BadCredentials desc="" retry_delay=0`로 응답 — **이건 프로토콜이 정상 작동한다는 뜻**(2FA 차단이나 프로토콜 거부가 아니라 순수 자격 증명 불일치)
3. **중단 시점의 다음 액션**: 사용자에게 `--username`이 로그인 이메일이 아니라 [spotify.com/account/overview](https://www.spotify.com/account/overview/)의 "사용자 이름"(username) 필드값인지 확인 요청 — 답변 대기 중 세션 중단. **다음 세션 시작 시 여기부터 재개**: 올바른 username으로 재시도 → 로그인 성공하면 `player.LoadTrack()` + Ogg Vorbis 디코딩까지 확인 → 성공 시 gomobile bind(이미 한 번 성공 확인됨, `Librespotmobile.xcframework` 생성 완료 이력 있음)로 넘어가 최소 Xcode 테스트 하니스 단계 진행
4. **주의**: 계정 자격 증명은 대화·문서 어디에도 남기지 않음(사용자가 별도 터미널에서 직접 입력) — 이 관례를 다음 세션에서도 유지할 것. 2FA 있는 메인 계정보다 테스트/부계정 권장(이미 사용자에게 안내함)
- [ ] SwiftUI + OpenAPI 생성 클라이언트, 라이브러리 뷰어 겸 원격 제어(기존 계획, 무수정)
...
- [ ] **(진행 중, 위 재개 체크리스트 참조)** `librespot-golang`+gomobile 임베딩 스파이크 — iOS 앱이 실제 Spotify Connect 기기로 인식되는지 검증하려던 원안은 기각(위 범위 정정 참조), 현재는 "미니 플레이어로 로그인+트랙 재생·디코딩이 되는지" 검증 중. gomobile 툴체인 자체는 정상 동작 확인(`gomobile bind -target ios ./librespotmobile`로 실제 `.xcframework` 생성 성공, 디바이스+시뮬레이터 슬라이스 포함). 순수 Go 레벨 로그인 프로토콜 핸드셰이크 검증 완료(2개 패치 필요, 위 참조) — 실 계정 로그인 성공 확인은 아직
- [ ] **(신규, 범위 변경)** 미니 플레이어 UI(검색·재생) — 원안의 "공식 앱에서 캐스팅"은 불가하므로, 곡 선택 UI를 자체 구현 필요(`librespotmobile`에 검색 API가 노출 안 돼 있어 바인딩 추가 작업 필요 — 원본 `librespot/mercury/api.go`의 `Search()`는 존재하나 mobile 바인딩 대상엔 없음)
- [ ] **(신규)** Ogg Vorbis 디코딩 — `MobileAudioFile.read()`가 반환하는 건 디코딩 전 원본 바이트(Ogg Vorbis 인코딩)이므로, iOS 쪽에서 별도 디코더 필요(AVFoundation은 Ogg Vorbis 미지원 — `stb_vorbis` 등 C 라이브러리 임베딩 검토)
- [ ] **(신규)** 자체 재생 중 디코딩 PCM을 원격 인제스트로 스트리밍(포그라운드 전용, 백그라운드 상주 미지원) — `/remote/audio/sessions`·`/chunk`·`/end`(Phase 8.5에서 이미 구현, 무수정)
...
| 2026-07-30 | **Phase 9 최종 전체 브랜치 리뷰 → fix wave**: Critical 1건 — `MusicnaApp.swift`가 `PlayerStatusStore`/`LiveAnalysisStore`를 생성만 하고 `start()`를 어디서도 호출하지 않아 메뉴바가 항상 "api 연결 안 됨"·"—" 상태로 고정되던 버그(`MenuBarExtra` 콘텐츠 뷰는 최초 클릭 시에만 인스턴스화되므로 뷰 쪽 `.task`로는 못 고침) → `MusicnaApp.init()`에서 두 스토어를 만든 직후 `Task { @MainActor in ... start() }`로 즉시 시작하도록 수정. 같은 리뷰에서 지목된 죽은 필드 `LiveAnalysisStore.isConnected`는 `handle(_:)` 최초 진입 시 `true`로 설정하도록 연결(삭제 대신 배선 선택 — "실시간 분석 연결 안 됨" 표시는 범용적으로 유용하다고 판단), 회귀 테스트 `testIsConnectedBecomesTrueAfterFirstEvent` 추가. Important 2건: ① `MenuBarView`의 재생 컨트롤에 "이전 곡" 버튼(`playerPrevious`)과 볼륨 `Slider`(`playerVolume`, 드래그 종료 시 1회 호출)를 추가 — 계획 마일스톤 문구("볼륨 조작")를 실제 UI가 충족 못 하던 문제 ② 설정 버튼이 macOS 13 이하 전용 셀렉터(`showPreferencesWindow:`)만 보내 이 머신의 실제 OS(macOS 26.5)에서 미동작 — `showSettingsWindow:`(14+)를 `NSApp.responds(to:)`로 먼저 확인 후 우선 시도, 없으면 폴백하도록 수정. `xcodegen generate` 재실행 후 `xcodebuild` `** BUILD SUCCEEDED **`, `swift test` 28 passed(27+신규 1) 확인 | 단일 커밋 `05e4cb5`. 스코프된 재리뷰 통과(새 Critical/Important 없음, Minor 1건 — 볼륨 슬라이더가 외부 변경과 재동기화 안 됨, non-blocking으로 park). 리뷰·수정 전 과정을 담은 SDD 워크스페이스(`.superpowers/sdd/2026-07-29-phase-9-macos-native-app/`)는 완료 후 관례대로 정리(git-ignored 디렉터리, 기록은 이 로그와 git 커밋 메시지가 대신함). 3건 모두 실기기 GUI로는 미검증(자동화 도구 부재) — 코드 읽기·빌드·유닛 테스트로만 확인, 실기기 검증은 여전히 Phase 9의 유일한 남은 항목 |
| 2026-07-30 | **Phase 9 → `main` 병합**: 최종 전체 브랜치 리뷰(opus) + fix wave 통과 후 사용자 선택으로 `phase-9-macos-native-app`를 `main`에 fast-forward 병합(커밋 `4a45c9e..4e546ff`). 병합 후 워크스페이스 전체(Python 232 passed·1 skipped, Swift 28 passed, `xcodebuild` BUILD SUCCEEDED) 재검증 통과. 브랜치 로컬·원격 삭제 | Phase 9 구현은 이것으로 `main` 기준 완료. 남은 건 실기기(macOS) 마일스톤 검증뿐(위 Phase 9 체크리스트 참조) — 사용자가 실기기 검증 전 병합을 선택함(CLAUDE.md의 "Phase 완료 시 main 병합" 관례를 그대로 따름, 검증 자체는 후속 세션 과제로 이월) |
| 2026-07-30 | **Alembic 마이그레이션 도입**(브랜치 `feature-alembic-migrations`, Task 1~4): `core/alembic.ini`+`core/src/musicna_core/store/migrations/`(env.py·script.py.mako) 스캐폴딩(`a328286`) → autogenerate로 초기 리비전 `3f5636cb793d` 생성, 기존 7개 테이블을 컬럼 단위로 대조해 손보정 없이 재현 확인(`2279ee0`) → `create_session_factory()`가 `Base.metadata.create_all()` 대신 `alembic upgrade head`를 호출하도록 교체, `create_all` 완전 제거·공개 시그니처는 그대로 유지(`36ea6d6`). 회귀 테스트 `core/tests/test_migrations.py` 2건 신규(모두 `tmp_path` 스크래치 DB만 사용). 설계: [2026-07-30-alembic-migrations-design.md](superpowers/specs/2026-07-30-alembic-migrations-design.md) | 워크스페이스 232 passed·1 skipped → 237 passed·0 skipped, 그러나 이 증가분이 전부 이 작업 때문은 아님: 이 브랜치가 새로 추가한 테스트는 2건뿐이고, 나머지(+3 passed / -1 skipped)는 Task 1의 `uv sync --all-packages --extra transcribe --extra analyze --extra mood` 실행이 이 머신에 torch/laion_clap을 설치하면서 `core/tests/test_moods.py`의 `pytest.importorskip("torch")`로 스킵되던 3건이 풀린, Alembic 작업과 무관한 부수 효과. 기존 `data/musicna.db`(실캡처 데이터 보관, git 미추적)에 대한 `alembic stamp head`는 이 계획의 범위 밖으로 남겨둠 — 이 브랜치가 리뷰·병합된 뒤 컨트롤러가 백업 후 사용자 확인을 받아 수동 실행 예정 |
| 2026-07-30 | **Alembic 마이그레이션 최종 리뷰(opus) → `main` 병합 → 기존 DB stamp**: 최종 전체 브랜치 리뷰 통과(wheel 패키징·경로 해석·리비전 발견까지 직접 검증, Ready to merge: Yes) — Important 1건은 코드 결함이 아니라 "기존 DB를 stamp하기 전에 api/batch를 재시작하면 table already exists로 크래시한다"는 운영 주의사항으로, 이미 계획에 반영돼 있던 수동 단계와 정확히 일치해 코드 수정 없이 확인만 함. `feature-alembic-migrations`를 `main`에 fast-forward 병합(커밋 `f5f3351..2d6bba2`), 워크스페이스 237 passed 재검증, 브랜치 정리. 이어서 사용자 확인 후 실제 `data/musicna.db` 백업(`data/musicna.db.bak`, 확인 후 삭제) → `MUSICNA_DB=../data/musicna.db uv run alembic -c alembic.ini stamp head` 실행 → `alembic_version` 테이블에 `3f5636cb793d` 1행 기록 확인, 기존 7개 테이블·데이터(트랙 2·분석 8) 그대로 확인, `create_session_factory("data/musicna.db")` 직접 호출로 에러 없이 통과(멱등적 upgrade head) 검증 | Alembic 마이그레이션 도입 전 과정 완료 — Phase 4 체크리스트의 마지막 남은 항목까지 해소. 앞으로 스키마 변경은 `cd core && alembic revision --autogenerate -m "..."`로 새 리비전을 작성하면 됨 |
| 2026-07-31 | **Phase 10(iOS) 스파이크 착수 — "librespot-golang+gomobile" 원안 재검토, 범위 정정**: 사용자 질문("아이폰에서 음악 분석 가능한가? 웹앱으로?")에서 출발 — iOS는 시스템 오디오 캡처 API 자체가 없어 웹앱 포함 어떤 방식으로도 "옆에서 캡처"는 불가하다는 기존 제약을 재확인. Phase 10 원안(iOS 앱이 Spotify Connect 수신 기기가 되어 공식 앱에서 캐스팅)을 실제로 검증하기 위해 `librespot-golang`을 clone해 `gomobile bind -target ios ./librespotmobile` 실행 → **놀랍게도 성공, 실제 `.xcframework`(디바이스+시뮬레이터) 생성 확인**. 그러나 생성된 Objective-C 헤더와 원본 Go 소스(`librespot/spirc/controller.go`)를 직접 추적한 결과, 이 라이브러리엔 Connect 프로토콜(SPIRC) **수신**(재생 명령을 받아 실행하는 쪽) 로직이 없고 `spirc.Controller`는 반대로 "남의 Connect 기기를 조종하는 리모컨"임을 확인 — 원안이 요구하는 핵심 기능이 애초에 존재하지 않음. 사용자와 대안 논의(② SPIRC 수신 로직 직접 구현 vs ③ 아이폰 앱 자체가 미니 플레이어가 되어 직접 재생) → 사용자가 **③ 선택** — 2번 대비 3번의 단점(사용 경험 단절, 검색/브라우징 UI 자체 구현 필요, 타인이 재생하는 음악은 못 잡음)을 먼저 설명하고 확인받음 | 범위 변경은 아직 설계 스펙 문서에 반영 전 — PROGRESS.md에만 정정 기록, 다음 세션에서 스펙 재작성 필요. `librespot-golang`의 gomobile 바인딩 API 노출 범위(로그인·메타데이터·트랙 다운로드만, 검색 미노출)도 이번에 확인 |
| 2026-07-31 | **최소 스파이크 — 순수 Go 레벨 로그인 프로토콜 검증**: 전체 iOS/Xcode 스캐폴딩 전에 가장 싼 방법(순수 Go CLI)으로 "로그인이 지금도 되는가"부터 검증하기로 함. `librespot-golang`의 `main.go`(완전한 예제 CLI, `vorbis-go/decoder`로 디코딩까지 포함) 빌드 시도 → `portaudio`/`libogg`/`libvorbis` Homebrew 설치 후 빌드 성공. 실행 결과 `dial tcp ...: connection refused`(AP 후보 중 하나가 응답 없음, 라이브러리가 재시도 안 함 — apresolve.spotify.com 자체와 반환된 AP 후보 6개 전부 TCP 접속 가능함을 직접 확인해 "완전히 막힘"이 아님을 검증) → 재시도로 `unexpected EOF`(핸드셰이크 중 연결 끊김) → 원인 추적: `session.go`의 `makeHelloMessage()`가 `Product_PRODUCT_PARTNER`(파트너 전용 클라이언트 타입)로 자신을 선언 중이었음, `Product_PRODUCT_CLIENT`로 패치 → 서버가 정상적인 로그인 응답을 주기 시작(`authentication failed`로 에러 종류 변경). 이어서 `login.go`의 `handleLogin()`이 `APLoginFailed`의 실제 에러 코드를 버리고 뭉뚱그리는 문제 발견 → `ErrorCode`/`ErrorDescription`/`RetryDelay`를 노출하도록 패치 → 사용자의 실제 계정으로 재시도 시 `code=BadCredentials`로 정확한 원인 확인(2FA 차단이나 프로토콜 거부가 아니라 순수 자격 증명 문제 — **프로토콜 레벨 검증은 사실상 통과**). username이 이메일이 아니라 Spotify 계정 설정의 "사용자 이름" 필드값이어야 할 가능성을 사용자에게 안내하고 재시도 요청 → **응답 대기 중 세션 중단**(다음 세션에서 재개, 위 Phase 10 체크리스트의 "스파이크 재개 체크리스트" 참조) | 스파이크 코드는 전부 레포 밖 스크래치 디렉터리(세션별 임시 경로)에만 존재 — 리포지토리에 커밋된 변경 없음, 재개 시 clone부터 다시 시작하고 문서화된 2개 패치를 재적용해야 함. 계정 자격 증명은 대화·문서 어디에도 남기지 않음(사용자가 별도 터미널에서 직접 입력하는 방식 유지) |
