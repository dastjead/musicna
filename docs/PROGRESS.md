...
- **다음 할 일 (원격 전용 작업이 필요하다면)**: Alembic 마이그레이션 구현은 완료(브랜치 `feature-alembic-migrations`, 아래 작업 로그·Phase 4 체크리스트 참조) — 남은 건 기존 `data/musicna.db`에 대한 `alembic stamp head` 수동 실행뿐(실캡처 데이터 보관 파일이라 사용자 확인 후 진행)
...
- [x] Alembic 마이그레이션 도입 — `core/src/musicna_core/store/migrations/`, `create_session_factory()`가 `create_all()` 대신 `alembic upgrade head`를 적용(`create_all` 호출은 완전히 제거, 공개 시그니처는 그대로). 초기 리비전(`3f5636cb793d`)이 기존 7개 테이블을 autogenerate로 그대로 재현(리뷰에서 컬럼 단위 대조, 손보정 불필요). 회귀 테스트: `core/tests/test_migrations.py`(2건). **주의**: 실캡처 데이터가 있는 기존 `data/musicna.db`에 대한 `alembic stamp head`(실제 ALTER 없이 "이미 적용됨" 표시)는 이 구현에 포함되지 않음 — 별도로 사용자 확인 후 수동 실행 필요(설계 스펙 참조)
